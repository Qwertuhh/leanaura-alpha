...
const notebookStore = (
    ...
    deleteNotebook: (slug: string) => {
        set((state) => {
            const notebooks = state.Notebooks || [];
            const filtered = notebooks.filter((n: Notebook) => n.slug !== slug);
    ...
    renameNotebook: (slug: string, newName: string) => {
        set((state) => {
            const notebooks =
                state.Notebooks?.map((n : Notebook) =>
                    n.slug === slug
    ...
    getNotebookByName: (notebookName: string) => {
        return get().Notebooks?.find((n : Notebook) => n.notebookName === notebookName);
    },

    getNotebookBySlug: (slug: string) => {
        return get().Notebooks?.find((n: Notebook) => n.slug === slug);
    },

    getNotebookNameBySlug: (slug: string) => {
        const notebook = get().Notebooks?.find((n: Notebook) => n.slug === slug);
