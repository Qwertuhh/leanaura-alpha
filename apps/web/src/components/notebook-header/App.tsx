...
function NotebookHeader({panelGroupRef, maximizePanel}: NotebookHeaderProps) {
    return (
        <header className="h-[var(--header-height)] w-full bg-stone-100 dark:bg-stone-900 border-b border-stone-200 dark:border-stone-700 p-2 px-4 flex items-center justify-between">
            ...
            <div className="flex flex-row gap-1 bg-stone-100 dark:bg-stone-800 bg-opacity-50 rounded p-2">
                <PanelButton tooltip="Toggle Editor" {...panelButtonProperty}
                             onClick={() => togglePanel(0, panelGroupRef)} onDoubleClick={() => maximizePanel(0)}><BookText {...panelButtonIconProperty}/></PanelButton>
                <PanelButton tooltip="Toggle Canvas" {...panelButtonProperty}
                             onClick={() => togglePanel(1, panelGroupRef)} onDoubleClick={() => maximizePanel(1)}><PenSquare {...panelButtonIconProperty}/></PanelButton>
                <PanelButton tooltip="Toggle Chat" {...panelButtonProperty}
                             onClick={() => togglePanel(2, panelGroupRef)} onDoubleClick={() => maximizePanel(2)}><MessageSquare {...panelButtonIconProperty}/></PanelButton>
            </div>
        </header>
    );
}
